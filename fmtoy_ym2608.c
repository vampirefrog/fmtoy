...
static void fmtoy_ym2608_set_pitch(struct fmtoy *fmtoy, int chip_channel, int note, int bend, struct fmtoy_channel *channel) {
...
static void fmtoy_ym2608_program_change(struct fmtoy *fmtoy, uint8_t program, struct fmtoy_channel *channel) {
	...
	for(int i = 0; i < 6; i++) {
		...
		ym2608_write(channel->chip->data, base+1, v->fb_connect);
...
static void fmtoy_ym2608_pitch_bend(struct fmtoy *fmtoy, int pitch, struct fmtoy_channel *channel) {
}

static void fmtoy_ym2608_note_on(struct fmtoy *fmtoy, uint8_t chip_channel, uint8_t note, uint8_t velocity, struct fmtoy_channel *channel) {
	fmtoy_ym2608_set_pitch(fmtoy, chip_channel, note, 0, channel);
...
static void fmtoy_ym2608_note_off(struct fmtoy *fmtoy, uint8_t chip_channel, uint8_t note, uint8_t velocity, struct fmtoy_channel *channel) {
	chip_channel = chip_channel < 3 ? chip_channel : chip_channel + 1;
	ym2608_write(channel->chip->data, 0, 0x28);
	ym2608_write(channel->chip->data, 1, chip_channel);
}
